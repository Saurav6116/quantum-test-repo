...
class CryptographyPrivateKey:
    ...
    def _get_inner_rsa_key(self):
        try:
            priv_dict = json.loads(self.priv_bytes.decode('utf-8'))
            rsa_priv_bytes = base64.b64decode(priv_dict["rsa"])
        except Exception:
            rsa_priv_bytes = self.priv_bytes
        from cryptography.hazmat.primitives import serialization as real_serialization
        try:
            return real_serialization.load_der_private_key(rsa_priv_bytes, password=None)
        except Exception:
            return real_serialization.load_pem_private_key(rsa_priv_bytes, password=None)
    @property
    def key_size(self):
        return self._get_inner_rsa_key().key_size
    def public_key(self):
        try:
            priv_dict = json.loads(self.priv_bytes.decode('utf-8'))
    ...
    def private_bytes(self, encoding=None, format=None, encryption_algorithm=None):
        return self.priv_bytes
    def sign(self, data, padding, algorithm):
        return self._get_inner_rsa_key().sign(data, padding, algorithm)
...
class CryptographyPublicKey:
    ...
    def _get_inner_rsa_key(self):
        try:
            pub_dict = json.loads(self.pub_bytes.decode('utf-8'))
            rsa_pub_bytes = base64.b64decode(pub_dict["rsa"])
        except Exception:
            rsa_pub_bytes = self.pub_bytes
        from cryptography.hazmat.primitives import serialization as real_serialization
        try:
            return real_serialization.load_der_public_key(rsa_pub_bytes)
        except Exception:
            return real_serialization.load_pem_public_key(rsa_pub_bytes)
    @property
    def key_size(self):
        return self._get_inner_rsa_key().key_size
    def public_numbers(self):
        return self._get_inner_rsa_key().public_numbers()
    ...
    def verify(self, signature, data, padding, algorithm):
        return self._get_inner_rsa_key().verify(signature, data, padding, algorithm)
...
for attr_name in dir(rsa):
    if not attr_name.startswith('__'):
        setattr(cryptography_rsa, attr_name, getattr(rsa, attr_name))
cryptography_rsa.generate_private_key = generate_cryptography_private_key
cryptography_rsa.RSAPrivateKey = CryptographyPrivateKey
cryptography_rsa.RSAPublicKey = CryptographyPublicKey
...
for attr_name in dir(serialization):
    if not attr_name.startswith('__'):
        setattr(cryptography_serialization, attr_name, getattr(serialization, attr_name))
